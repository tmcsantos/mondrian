...
public class FunctionTest extends FoodMartTestCase {
    ...
    public void testCountWithOptimizedNonEmpty() {
        if (!MondrianProperties.instance().EnableNativeNonEmptyFun.get()) {
            // No point testing these if the native filters
            // are turned off.
            return;
        }
        propSaver.set(MondrianProperties.instance().CellBatchSize, 10);

        final String query =
            "with "
            + "member [Measures].[c] as '"
            + "count(product.[product name].members, excludeempty)'\n"
